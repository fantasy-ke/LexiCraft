<template>
  <div
      class="min-h-screen bg-slate-50 text-slate-900 overflow-hidden font-sans selection:bg-blue-600 selection:text-white relative">

    <!-- Dynamic Background Elements -->
    <!-- 增强了颜色深度和不透明度，移除了混合模式以确保在浅色背景下可见 -->
    <div class="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
      <div
          class="absolute top-[-10%] left-[-10%] w-96 h-96 bg-blue-400 rounded-full filter blur-[80px] opacity-40 animate-blob"></div>
      <div
          class="absolute top-[-10%] right-[-10%] w-96 h-96 bg-indigo-400 rounded-full filter blur-[80px] opacity-40 animate-blob animation-delay-2000"></div>
      <div
          class="absolute -bottom-32 left-20 w-96 h-96 bg-pink-400 rounded-full filter blur-[80px] opacity-40 animate-blob animation-delay-4000"></div>
    </div>
    ...
    <header class="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
      <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="hidden md:grid grid-cols-3 items-center h-16">
          ...
          <div class="flex items-center space-x-3 cursor-pointer justify-self-start" @click="router.push('/')">
            <div
                class="w-8 h-8 bg-gradient-to-br from-blue-600 to-indigo-600 rounded-lg flex items-center justify-center shadow-lg shadow-blue-500/20">
          ...
          <div class="flex items-center justify-center space-x-8 justify-self-center w-full">
            <button class="bg-transparent border-none outline-none focus:outline-none focus:ring-0 cursor-pointer text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors"
            ...
            <a class="text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors" href="/articles">{{
              t[lang].nav.articles }}</a>
            <a class="text-sm font-medium text-slate-500 hover:text-blue-600 transition-colors" href="/words">{{
              t[lang].nav.words }}</a>
          </div>
        ...
        <div class="md:hidden flex justify-between items-center h-16">
          ...
          <div class="flex items-center space-x-3">
            ...
            <button class="text-slate-500 hover:text-blue-600 p-2 border-none outline-none focus:ring-0"
      ...
      <div v-show="showMobileMenu"
           class="md:hidden fixed inset-x-0 top-16 bottom-0 z-40 bg-white/95 backdrop-blur-xl border-t border-slate-200 p-6 transition-all duration-300 ease-in-out">
        <div class="flex flex-col space-y-6">
          <button class="text-lg font-medium text-slate-600 hover:text-blue-600 transition-colors text-left border-none outline-none bg-transparent"
                  @click="handleMobileNavClick('hero')">
            {{ t[lang].nav.home }}
          </button>
          <button class="text-lg font-medium text-slate-600 hover:text-blue-600 transition-colors text-left border-none outline-none bg-transparent"
          ...
          <a class="text-lg font-medium text-slate-600 hover:text-blue-600 transition-colors" href="/articles">{{
            t[lang].nav.articles }}</a>
          <a class="text-lg font-medium text-slate-600 hover:text-blue-600 transition-colors" href="/words">{{
            t[lang].nav.words }}</a>
          <div class="h-px bg-slate-200 my-2"></div>
          <button class="text-lg font-medium text-slate-600 hover:text-blue-600 text-left transition-colors border-none outline-none bg-transparent"
                  @click="handleLogin">
            {{ t[lang].nav.login }}
          </button>
          <button class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-xl text-lg font-bold text-center shadow-lg shadow-blue-500/20 transition-all border-none outline-none"
                  style="border: none !important;"
                  @click="startLearning">{{ t[lang].nav.start }}
          </button>
        </div>
      </div>
    </header>
    ...
    <main class="pt-24 pb-12 relative z-10">
      ...
      <section id="hero" class="relative px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center min-h-[70vh]">
        <div class="relative z-10 text-center max-w-5xl mx-auto space-y-8">
          <div
              class="inline-flex items-center space-x-2 bg-blue-50 border border-blue-100 rounded-full px-4 py-1.5 mb-4 animate-fade-in backdrop-blur-sm bg-opacity-80">
      ...
      <section id="features" class="py-20 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
          <div class="flex items-center justify-between mb-8">
            ...
            <a class="text-sm text-slate-500 hover:text-blue-600 flex items-center gap-1 transition-colors" href="#">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              <div
                  class="aspect-video bg-gradient-to-br from-slate-50 to-slate-100 group-hover:from-slate-100 group-hover:to-slate-200 transition-colors p-6 flex items-center justify-center relative">
                ...
                <div
                    ...
                    class="w-16 h-16 rounded-full flex items-center justify-center transition-transform duration-300 group-hover:scale-110"
                >
                  <!-- Icons based on index -->
                  <svg v-if="index === 0" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <svg v-if="index === 1" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" stroke-linecap="round" stroke-linejoin="round"
                          stroke-width="2"></path>
                  </svg>
                  <svg v-if="index === 2" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-linecap="round" stroke-linejoin="round"
                          stroke-width="2"></path>
                  </svg>
                  <svg v-if="index === 3" class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <footer class="border-t border-slate-200 bg-white/80 backdrop-blur-sm py-12 px-4 relative z-10">
      <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div class="flex items-center space-x-6">
          <a class="text-slate-500 hover:text-blue-600 transition-colors" href="https://github.com/zyronon/LexionCraft"
             target="_blank">
            GitHub
          </a>
          <a class="text-slate-500 hover:text-blue-600 transition-colors" href="#">
