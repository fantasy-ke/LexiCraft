...
<template>
  <div>
    <SettingItem desc="开启后，练习中会在上方显示上一个/下一个单词"
                 title="显示上一个/下一个单词"
    >
      <Switch v-model="settingStore.showNearWord"/>
    </SettingItem>

    <SettingItem desc="在词典详情页面，点击学习按钮后，是否显示练习设置弹框"
                 title="不默认显示练习设置弹框"
    >
    ...
    <SettingItem class="gap-0!" title="单词循环设置">
      ...
      <div v-if="settingStore.repeatCount === 100" class="ml-2 center gap-space">
        <span>循环次数</span>
    ...
    <SettingItem desc="复习词与新词的比例，修改后下次学习生效"
                 title="复习比"
    >
      <InputNumber v-model="settingStore.wordReviewRatio" :max="10" :min="0"/>
    </SettingItem>
    ...
    <SettingItem title="倍速">
      <Slider v-model="settingStore.wordSoundSpeed" :max="3" :min="0.5" :step="0.1" showText showValue/>
    </SettingItem>
    <div class="line"></div>
    ...
    <SettingItem desc="仅在 **跟写** 时生效，听写、自测、默写均不会自动切换，需要手动按 **空格键** 切换"
                 title="自动切换下一个单词"
    >
      <Switch v-model="settingStore.autoNextWord"/>
    </SettingItem>

    <SettingItem desc="正确输入单词后，自动跳转下一个单词的时间"
                 title="自动切换下一个单词时间"
    >
    ...
    <SettingItem title="外语字体">
      <Slider
          ...
          :max="100"
          :min="10" showText showValue unit="px"/>
    </SettingItem>
