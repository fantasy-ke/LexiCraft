...
public static class PathMatcher
{
    ...
    public static bool Matches(string requestPath, string pathPattern)
    {
        ...
        // 处理精确匹配
        if (pathPattern.Equals(requestPath, StringComparison.OrdinalIgnoreCase)) return true;
    ...
    private static bool MatchParameterizedPath(string requestPath, string pathPattern)
    {
        ...
        for (var i = 0; i < requestSegments.Length; i++)
        {
            ...
            if (patternSegment.StartsWith("{") && patternSegment.EndsWith("}")) continue; // 参数段总是匹配

            // 否则进行精确匹配
            if (!string.Equals(patternSegment, requestSegment, StringComparison.OrdinalIgnoreCase)) return false;
        }
