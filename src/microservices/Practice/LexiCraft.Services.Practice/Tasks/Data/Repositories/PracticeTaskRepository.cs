        ...
        IPracticeTaskRepository
{
    ...
    public async Task<List<PracticeTask>> GetActiveTasksForUserAsync(string userId, CancellationToken cancellationToken = default)
    {
        return await FindAsync(x => x.UserId == userId && x.Status == PracticeStatus.InProgress, cancellationToken);
    }

    public async Task<List<PracticeTask>> GetCompletedTasksAsync(string userId, int limit = 10, CancellationToken cancellationToken = default)
    {
        var (items, _) = await FindPagedAsync(
            filter: x => x.UserId == userId && x.Status == PracticeStatus.Finished,
            skip: 0,
            limit: limit,
            sortBy: x => x.FinishedAt!,
            sortDescending: true,
    ...
    public async Task<List<PracticeTask>> GetTasksByTypeAsync(string userId, PracticeTaskType taskType, CancellationToken cancellationToken = default)
    {
        return await FindAsync(x => x.UserId == userId && x.TaskType == taskType, cancellationToken);
    }

    public async Task<List<PracticeTask>> GetTasksBySourceAsync(string userId, PracticeTaskSource sourceType, CancellationToken cancellationToken = default)
    {
