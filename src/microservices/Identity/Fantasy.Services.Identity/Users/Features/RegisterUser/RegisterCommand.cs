...
using Fantasy.Services.Identity.Identity.Internal.Commands;
using Fantasy.Services.Identity.Identity.Models.Enum;
using Fantasy.Services.Identity.Shared.Contracts;
using Fantasy.Services.Identity.Shared.Dtos;
using Fantasy.Services.Identity.Users.Internal.Commands;
using MediatR;

namespace Fantasy.Services.Identity.Users.Features.RegisterUser;
    ...
    : ICommandHandler<RegisterCommand, TokenResponse>
{
    public async Task<TokenResponse> Handle(RegisterCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // 注册成功后自动登录逻辑
            return await mediator.Send(new GenerateTokenResponseCommand(user, "Register", "注册成功并登录"),
                cancellationToken);
        }
        catch (Exception e)
        {
