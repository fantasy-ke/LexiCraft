...
internal sealed class RedisDistributedLockProvider : IDistributedLockProvider
{
        ...
        do
        {
            ...
            try
            {
                ...
                var result = await database.ScriptEvaluateAsync(
                    AcquireLockScript,
                    [fullLockKey],
                    [lockValue, lockTimeoutMs]).WaitAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var exists = await database.KeyExistsAsync(fullLockKey).WaitAsync(cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var deleted = await database.KeyDeleteAsync(fullLockKey).WaitAsync(cancellationToken);
