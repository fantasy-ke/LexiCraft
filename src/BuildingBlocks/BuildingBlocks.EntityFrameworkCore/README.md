...
该组件实现 EF Core 通用仓储、工作单元、审计拦截器和强类型 ID 转换器，不包含具体数据库提供程序。

公共仓储与工作单元接口位于 `BuildingBlocks.Persistence.Abstractions`；本项目只提供 EF 实现。

## 目录职责

- `Abstractions/`：EF 专属端口，例如 `IDataSeeder<TContext>`。
- `Repositories/`：`QueryRepository<TDbContext,TEntity>` 与 `Repository<TDbContext,TEntity>`。
- `Transactions/`：`UnitOfWork<TDbContext>`。
- `Interceptors/`：保存管线拦截器。
- `Converters/`：EF 值转换器。
- `Extensions/`：DI、ModelBuilder 和 IQueryable 扩展。

目录与命名空间保持一致，不再把接口、仓储、工作单元全部堆在项目根目录。
...
`WithDbAccess<TDbContext>` 会以 scoped 生命周期注册 `AuditableEntityInterceptor` 和 `IUnitOfWork`。仓储扫描默认只检查 `TDbContext` 所在程序集，避免扫描全部引用程序集。
...
- 删除未实现 `ISoftDeleted` 的实体：仍按 EF Core 普通删除语义处理。
