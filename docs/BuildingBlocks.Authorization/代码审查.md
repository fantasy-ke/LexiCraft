...
> **命名更新（2026-08-18）**：本文写于改名之前，正文中的历史名称已不存在，对应关系为
> `LexiCraft.Shared` → `Fantasy.Shared`、`LexiCraftPermissionDefinitionProvider` → `FantasyPermissionDefinitionProvider`、
> `LexiCraft.Services.Identity{,.Api}` → `Fantasy.Services.Identity{,.Api}`、`src/LexiCraft.slnx` → `src/Fantasy.slnx`（测试为 `src/Fantasy.Tests.slnx`）。
> 文中记录的验证命令与结果按当时事实保留，不做回溯改写。Vocabulary、Practice 仍保持 `LexiCraft.*` 命名。
...
## 10. 2026-08-14 类库边界复审与补充重构

本轮在既有授权链路审查基础上继续检查了项目引用、目录、命名空间、公共 API 和缓存底层行为，补充结论如下。

### 10.1 已修复遗漏

1. **授权核心传递依赖 Redis。** 原 Redis 缓存、会话和同步实现已迁移到 `BuildingBlocks.Authorization.Redis`；授权核心不再引用缓存项目。
2. **EF Core 为 `IUserContext` 依赖授权项目。** 接口已下沉到 `BuildingBlocks.Contexts`，EF Core 仅引用通用核心。
3. **管理员可旁路未知权限。** 权限存在性现在先于管理员角色判断，拼写错误或未注册权限统一拒绝。
4. **非法 Authorization Header 被转发。** 业务服务只转发参数非空的 Bearer Header，其余情况本地返回无效会话。
5. **权限定义管理器使用服务定位器。** 已改为直接注入 `IEnumerable<PermissionDefinitionProvider>`。
6. **Redis 配置和键污染核心模型。** `AuthorizationRedisOptions`、`AuthorizationRedisKeys` 已归入 Redis 适配层。
7. **实现类公共面过大。** Policy Handler、权限检查、JWT/UserContext 实现及 Redis 适配实现已收口为内部类型。
8. **授权核心存在无用直接包引用。** 已移除 `Newtonsoft.Json`；业务服务和 EF 项目依赖树需继续保持无 Redis/MemoryPack。

### 10.2 与缓存基库联动修复

授权 Redis 依赖的缓存类库同时修复了显式命中、锁后二次读取配置、本地实例隔离、取消传播、连接并发、Hash 时间戳和 Hash TTL 事务等问题。这样授权权限快照不会再受值类型误判、错误实例读取或永久 Hash 的底层缺陷影响。

### 10.3 仍需单独验证

1. 本轮没有运行真实 Redis/Aspire 多副本环境，不能证明登录、刷新、登出、权限撤销和故障切换的端到端行为。
2. 业务服务每个受保护请求同步调用 Identity 的容量和尾延迟需要压测数据，不能凭静态代码决定缓存或事件化方案。
3. Identity 内部权限验证端点仍应增加服务身份或由服务网格强制隔离。
4. 对称 JWT 密钥、Token HMAC、命名空间统一和配置全面校验均应拆成独立任务，避免与本轮边界重构混合。

详细目录、接口设计、兼容性和后续任务见同目录 `方案设计.md`。
### 10.4 本轮验证结果

| 验证 | 结果 |
| --- | --- |
| Caching 单元测试 | 7/7 通过 |
| Authorization 单元测试 | 20/20 通过 |
| Persistence 回归测试 | 30/30 通过 |
| `dotnet build src\LexiCraft.slnx --no-restore` | 通过，0 warning / 0 error |
| `dotnet test src\LexiCraft.slnx --no-build` | 57/57 通过；活动解决方案已补入 Authorization.Tests |
| 核心/Authorization/EF/Files 传递依赖检查 | 未发现 StackExchange.Redis 或 MemoryPack |
| UTF-8 损坏扫描 | 通过 |
| 真实 Redis/Aspire/负载验证 | 未执行，作为后续独立任务 |
